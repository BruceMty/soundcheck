...
int j;
   ...
   err = Pa_Initialize();
std::cout << "pa_initialize err: " << err << "\n";
   if( err != paNoError ) goto error;

   // show number of available devices
   std::cout << "number of pa input devices: " << Pa_GetDeviceCount() << "\n";
...
//      fflush(stdout);
      ...
      if( nearestNoteDelta == 0 || centsSharp >= 0 ) {
         ...
         for( i=0; i<chars+centsSharp; ++i )
            printf( " " );
         for( i=chars+centsSharp<0?0:chars+centsSharp; i<chars; ++i )
            printf( "=" );
      }
      printf( " %2s ", nearestNoteName );
      if( nearestNoteDelta != 0 )
         for( i=0; i<chars && i<centsSharp; ++i )
           printf( "=" );
      printf("\n");
   }
